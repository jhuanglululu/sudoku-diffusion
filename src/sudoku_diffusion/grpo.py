...
(clue counts from grpo_clue_counts; 0 = empty board), roll out `group_size`
...
from .data import MASK, make_puzzle, orbit_split, solved, validity_score
...
def reward(final_board: np.ndarray, puzzle: np.ndarray, steps_used: int, cfg) -> tuple[float, bool]:
...
def sample_puzzles(train_sols: np.ndarray, cfg, rng: np.random.Generator) -> np.ndarray:
    out = []
    while len(out) < cfg.puzzles_per_batch:
        n_clues = int(rng.choice(cfg.grpo_clue_counts))
        if n_clues == 0:
...
def train_grpo(model_name: str, training_name: str, seed: int) -> None:
    ...
    assert cfg.kind == "grpo" and cfg.init_from_training
    seed_all(seed)
    device = get_device()
    rng = np.random.default_rng(seed)
    gen = torch.Generator(device=device).manual_seed(seed)

    train_sols, _ = orbit_split(np.random.default_rng(12345))
    ...
    for step in bar:
        ...
        if (step + 1) % cfg.log_every == 0:
            rec.write(
                ...
                lr=cfg.lr, sec_per_step=round((time.time() - t0) / (step + 1), 3),
            )
        ...
        if (step + 1) % cfg.val_every == 0 or step + 1 == cfg.steps:
            ...
            save_checkpoint(model, model_name, training_name, seed, step + 1, {"solve_rate": solve_rate})
